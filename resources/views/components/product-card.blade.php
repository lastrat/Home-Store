        ...
        <div class="actions">
            @if($product->stock > 0)
                <form method="POST" action="{{ route('cart.add', $product) }}" class="inline">
                    @csrf
                    <button type="submit" class="cart-add-btn" title="Ajouter au panier">
            ...
            @endif
            <button onclick="event.preventDefault(); document.getElementById('wishlist-form-{{ $product->id }}').submit();" class="{{ $product->wishlists->isNotEmpty() ? 'active' : '' }}" title="Coup de cœur">
            ...
            <form id="wishlist-form-{{ $product->id }}" method="POST" action="{{ route('wishlist.toggle', $product) }}" class="hidden">
                @csrf
            </form>
        </div>
    </div>
